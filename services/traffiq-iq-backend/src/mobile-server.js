...
app.get('/health', async (_req, res, next) => {
  try { res.json({ ok: true, service: 'traffiq-android-adapter', dbTime: await checkDatabase() }); }
...
app.use((req, res) => res.status(404).json({ data: null, error: { code: 'not_found', message: `No mobile route for ${req.path}`, details: null } }));
...
async function start() {
  ...
  await checkDatabase();
  app.listen(port, '0.0.0.0', () => console.log(`TraffIQ Android compatibility API listening on :${port}`));
}

start().catch(async error => {
  console.error('failed to start TraffIQ Android compatibility API', error);
  await pool.end();
  process.exit(1);
});
